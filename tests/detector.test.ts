...
describe("detectBodyType", () => {
  ...
  it("returns unknown when no signals are found", () => {
    const detection = detectBodyType([file("meshes/armor/steel_armor.nif", "plain armor")]);
...
describe("createConversionPlan", () => {
  it("adds target specific operations for 3ba", () => {
    const detection = detectBodyType([file("meshes/3ba/body_0.nif", "3bbb body")]);
    const plan = createConversionPlan(detection, "3ba", [file("meshes/3ba/body_0.nif")]);

    expect(plan.operations.some((operation) => operation.id === "physics")).toBe(true);
  });
});
