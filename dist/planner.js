...
function relationshipOperations(sourceType, targetType) {
    ...
    if (sourceInfo.topology !== targetInfo.topology) {
        if (sourceInfo.topology === "vanilla") {
            ...
            ops.push({
                ...
                description: `Source topology '${sourceInfo.topology}' differs from target topology '${targetInfo.topology}'. ` +
    ...
    if (!sourceInfo.physicsSupport &&
        ...
        const more = targetInfo.physicsBones.length > 6
            ? ` …and ${targetInfo.physicsBones.length - 6} more`
            : "";
        const configType = targetInfo.cbpcCompatible && targetInfo.hdtSmpCompatible
    ...
    if (sourceInfo.physicsSupport &&
        ...
        ops.push({
            ...
            description: `Both bodies support physics but use different bone naming conventions (${sourceType.toUpperCase()} vs ${targetType.toUpperCase()}). ` +
...
export function createConversionPlan(detection, targetType, files) {
    ...
    if (packaging.fomod) {
        warnings.push("FOMOD installer structure detected. SlideSmith keeps installer metadata and remaps embedded Data roots for direct MO2/Vortex deployment.");
    }
    if (packaging.mo2) {
        warnings.push("MO2 metadata detected. Output stays in canonical Data-root paths so the converted mod remains Mod Organizer 2 ready.");
    }
    if (packaging.vortex) {
        warnings.push("Vortex metadata detected. Output uses canonical Skyrim Data layout so converted assets remain Vortex deployment friendly.");
    }
    ...
    if (missingWeightPairCount > 0) {
        warnings.push(`Detected ${missingWeightPairCount} mesh file(s) with only one Skyrim weight variant. Skyrim SE expects paired _0/_1 meshes for weight-slider support; missing counterparts will be auto-synthesized during conversion. For best morph-slider quality, re-export both weights from the source tool.`);
    }
