...
function targetSpecificOperations(targetType: BodyType): ConversionOperation[] {
  ...
  if (targetType === "bodytalk") {
    ops.push({
      ...
      description:
        "BodyTalk outputs often keep older male slider naming. Align chest, abdomen, and thigh proportions to the BodyTalk reference and confirm the generated slider-set naming matches BodyTalk conventions.",
    });
  }
  ...
  if (targetType === "sos") {
    ops.push({
      ...
      description:
        "Validate pelvis seam edge loops and partition slot SBP_52. SOS injects a genital mesh; any vertex overlap in the partition area will cause visible tearing at runtime.",
    });
  }
  ...
  if (targetType === "sam") {
    ops.push({
      ...
      description:
        "SAM uses per-morph weight and muscle multipliers. Export morphs as SAM-compatible .tri deltas and confirm the SAMLightBodyConfig.json references the new outfit.",
    });
  }
  ...
  if (targetType === "3ba") {
    ops.push({
      ...
      description:
        "3BA supports a belly physics chain in addition to breast/butt. Confirm belly bone (NPC Belly) is weighted and present in the CBPC config for full 3BA physics support.",
    });
  }
